...
// Agregar un test para diccionario con clave string y valor bool.
    ...
    d.definir("Migue", true);
    d.definir("Pepe", false);
    ASSERT_TRUE(d.def("Migue"));
    ASSERT_TRUE(d.def("Pepe"));
    ASSERT_FALSE(d.def("Juan"));
    ASSERT_EQ(d.obtener("Migue"), true);
    ASSERT_EQ(d.obtener("Pepe"), false);
    d.definir("Migue", false);
    ASSERT_TRUE(d.def("Migue"));
    ASSERT_EQ(d.obtener("Migue"), false);
}
#endif
