...
struct Ext2FSInode * Ext2FS::load_inode(unsigned int inode_number) {
	...
	unsigned int block_number = blockgroup_for_inode(inode_number);
	struct Ext2FSBlockGroupDescriptor *group = block_group(block_number);

	unsigned int inode_size = superblock()->inode_size;

	unsigned char *buffer_inodes = new unsigned char[1024];
	unsigned int offset = (inode_index / 8);
	read_block(group->inode_table + offset, buffer_inodes);

	unsigned int pos = (inode_index % 8) * 128;
	unsigned char *inode = new unsigned char[inode_size];

	for (unsigned int i = pos; i < pos + inode_size; i++) {
		inode[i - pos] = buffer_inodes[i];
	}

	delete[] buffer_inodes;

	return (struct Ext2FSInode *)inode;
}

unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
	unsigned int block_size = 1024 << _superblock->log_block_size;
	unsigned int block_length = block_size / 4;
	unsigned int single_indirect_limit = 12 + block_length;
	unsigned int double_indirect_limit = single_indirect_limit + block_length * block_length;
	unsigned int block_address = 0;

	//TODO: Ejercicio 1
	if (block_number < 12) {
		block_address = inode->block[block_number];
	} else if (block_number < single_indirect_limit) {
		unsigned int block_index = block_number - 12;
		unsigned char *buffer_single_indirect = new unsigned char[block_size];

		read_block(inode->block[12], buffer_single_indirect);
		block_address = ((unsigned int *)buffer_single_indirect)[block_index];

		delete[] buffer_single_indirect;
	} else if (block_number < double_indirect_limit) {
		unsigned char *buffer_single_indirect = new unsigned char[block_size];

		unsigned char *buffer_doble_indirect = new unsigned char[block_size];
		read_block(inode->block[13], buffer_doble_indirect);

		unsigned int double_table_block_index = (block_number - single_indirect_limit) / block_length;
		unsigned int single_indirect_index = ((unsigned int *)buffer_doble_indirect)[double_table_block_index];

		unsigned int single_table_block_index = (block_number - single_indirect_limit) % block_length;

		read_block(single_indirect_index, buffer_single_indirect);

		block_address = buffer_single_indirect[single_table_block_index];

		delete[] buffer_doble_indirect;
		delete[] buffer_single_indirect;
	}
...
struct Ext2FSInode * Ext2FS::get_file_inode_from_dir_inode(struct Ext2FSInode * from, const char * filename)
{
	...
	Ext2FSDirEntry *directories = 0;
	Ext2FSDirEntry *directory;

	unsigned int inode_value = 0;
	unsigned int directory_index = 0;
	unsigned int block_number = 0;
	bool file_found = false;
	unsigned char *block_buffer = new unsigned char[block_size];

	while (!file_found && block_number < double_indirect_limit) {
		unsigned int block_address = get_block_address(from, block_number);
		read_block(block_address, block_buffer);
		directories = (Ext2FSDirEntry *) block_buffer;

		directory_index = 0;
		while (!file_found && directory_index < 1024) {
			directory = directories[directory_index];

			if (strncmp(filename, directory->name, directory->name_length) == 0 && strlen(filename) == directory->name_length) {
				file_found = true;
				inode_value = directory->inode;
			}

			directory_index += directory->record_length;
		}

		block_number++;
	}

	std::cout << "inode value!!: " << inode_value << std::endl;
	return load_inode(inode_value);
}
