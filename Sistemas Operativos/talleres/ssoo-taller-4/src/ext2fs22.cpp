...
struct Ext2FSInode * Ext2FS::load_inode(unsigned int inode_number)
{
	...
	unsigned int block_number = offset_for_inode / (block_size/_superblock->inode_size);
	unsigned int block_offset = offset_for_inode % (block_size/_superblock->inode_size);
...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
    ...
    // Primera indirección
		if(block_number < 268){
			unsigned int block_address = inode->block[12]; //entrada 13
			read_block(block_address, block_buf);
			result = ((unsigned int *) block_buf)[block_number - 12];

    // Segunda indirección
		}else{
			block_number = block_number - 268;
			unsigned int offset= block_number % 256;
			unsigned int block = block_number /256;
			unsigned int block_address = inode->block[13]; //entrada 14
			read_block(block_address, block_buf);
			block = ((unsigned int *) block_buf)[block];
...
struct Ext2FSInode * Ext2FS::get_file_inode_from_dir_inode(struct Ext2FSInode * from, const char * filename)
{
	...
	while (1){
		if(leer > block_size){
			i++;
			leer = leer-block_size;

			block_address = get_block_address(from, i);
			block_address2 = get_block_address(from, i+1);

			read_block(block_address, block_buf);
			read_block(block_address2, block_sig);
		}

		Ext2FSDirEntry *entradaActual = (Ext2FSDirEntry*) (block_buf + leer);
		int tamanio = entradaActual->record_length;
		int longitud = entradaActual->name_length;

		char* filename2= entradaActual->name;
		if(longitud == strlen(filename) && !strncmp(filename,filename2,longitud)){
            unsigned int inodoActual = entradaActual->inode;
			free(block_buf);
            return load_inode(inodoActual);
		}

		leer = leer + entradaActual->record_length;
	}
	printf("nope \n");
	return NULL;
}
